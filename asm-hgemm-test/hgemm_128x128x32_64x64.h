...
__global__ void hgemm_128x128x32_64x64_32x32_db_naive(
    const half *a, const half *b, float *c, int M, int N, int K) {
    ...
    __shared__ half s_a[2][128][32];
    __shared__ half s_b[2][128][32];
...

    {
        half4x2 ldg_a[2], ldg_b[2];
        for (int ii = 0; ii < 2; ii++) {
            ldg_a[ii] = *(half4x2 *)&a[OFFSET(ldg_m + ii*64, ldg_k, K)];
            ldg_b[ii] = *(half4x2 *)&b[OFFSET(ldg_n + ii*64, ldg_k, K)];
        }

        for (int ii = 0; ii < 2; ii++) {
            *(half4 *)&s_a[0][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k    )] = ldg_a[ii].x;
            *(half4 *)&s_a[0][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k + 4)] = ldg_a[ii].y;
            *(half4 *)&s_b[0][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k    )] = ldg_b[ii].x;
            *(half4 *)&s_b[0][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k + 4)] = ldg_b[ii].y;
        }
    ...
    for (int ib = 1; ib < K/32; ib++) {
        ...
        for (int ii = 0; ii < 2; ii++) {
            for (int kk = 0; kk < 4; kk++) {
                tile_a[ii][kk] = *(half4 *)&s_a[(ib-1)%2][lds_m + ii*32][SWZ_32(lds_m + ii*32, lds_k + kk*8)];
            }
        }
        for (int ii = 0; ii < 2; ii++) {
            for (int kk = 0; kk < 4; kk++) {
                tile_b[ii][kk] = *(half4 *)&s_b[(ib-1)%2][lds_n + ii*32][SWZ_32(lds_n + ii*32, lds_k + kk*8)];
            }
        }
        for (int ii = 0; ii < 2; ii++) {
            for (int jj = 0; jj < 2; jj++) {
        ...
        for (int ii = 0; ii < 2; ii++) {
            *(half4 *)&s_a[ib%2][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k    )] = ldg_a[ii].x;
            *(half4 *)&s_a[ib%2][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k + 4)] = ldg_a[ii].y;
            *(half4 *)&s_b[ib%2][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k    )] = ldg_b[ii].x;
            *(half4 *)&s_b[ib%2][sts_mn + ii*64][SWZ_32(sts_mn + ii*64, sts_k + 4)] = ldg_b[ii].y;
        }
...

    {
        for (int ii = 0; ii < 2; ii++) {
            for (int kk = 0; kk < 4; kk++) {
                tile_a[ii][kk] = *(half4 *)&s_a[1][lds_m + ii*32][SWZ_32(lds_m + ii*32, lds_k + kk*8)];
            }
        }
        for (int ii = 0; ii < 2; ii++) {
            for (int kk = 0; kk < 4; kk++) {
                tile_b[ii][kk] = *(half4 *)&s_b[1][lds_n + ii*32][SWZ_32(lds_n + ii*32, lds_k + kk*8)];
            }
        }
        for (int ii = 0; ii < 2; ii++) {
            for (int jj = 0; jj < 2; jj++) {
